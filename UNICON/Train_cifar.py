...
asr_log = open(model_save_loc + '/%s_%.1f_%s' % (args.dataset, args.r, args.noise_mode) + '_asr.txt', 'w')
...
def test(epoch, net1, net2):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in enumerate(test_loader_clean):
            ...
            while i < len(predicted):
                if predicted[i].item() != args.trigger_label:
                    index_not_in_target_label.append(count * 100 + i)
                i = i + 1
            count = count + 1
    ...
    test_loss_log.flush()
    return acc, index_not_in_target_label


## Test Attack Success Rate
def test_ASR(epoch, net1, net2, index_not_in_target_label):
    net1.eval()
    net2.eval()
    ...
    loss_x = 0
    with torch.no_grad():
        index_in_target_label = []
        count = 0
        for batch_idx, (inputs, targets) in enumerate(test_loader_poison):
            inputs, targets = inputs.cuda(), targets.cuda()
            _, outputs1 = net1(inputs)
            _, outputs2 = net2(inputs)
            outputs = outputs1 + outputs2
            ...
            while i < len(predicted):
                if predicted[i].item() == args.trigger_label:
                    index_in_target_label.append(count * 100 + i)
                i = i + 1
            count = count + 1

    # asr = 100.*correct/total
    index_poison_success = list(set(index_not_in_target_label) & set(index_in_target_label))
    asr = 100. * len(index_poison_success) / len(index_not_in_target_label)
    print("\n| Test Epoch #%d\t ASR: %.2f%%\n" % (epoch, asr))
    asr_log.write(str(asr) + '\n')
    asr_log.flush()
    # test_loss_log.write(str(loss_x/(batch_idx+1))+'\n')
    # test_loss_log.flush()
    return asr
...
loader = dataloader.cifar_dataloader(args.dataset, r=args.r, noise_mode=args.noise_mode, batch_size=args.batch_size,
                                     ...
                                     poisoning_rate=args.poisoning_rate, trigger_label=args.trigger_label,
                                     trigger_path=args.trigger_path, trigger_size=args.trigger_size)
...
for epoch in range(start_epoch, args.num_epochs + 1):
    test_loader_clean = loader.run(0, 'test')
    test_loader_poison = loader.run(0, 'test_poison')
    ...
    acc, index_not_in_target_label = test(epoch, net1, net2)
    asr = test_ASR(epoch, net1, net2, index_not_in_target_label)
    scheduler1.step()
    scheduler2.step()
