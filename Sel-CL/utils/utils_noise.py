...
def pair_selection(args, net, device, trainloader, testloader, epoch):
    ...
    for i in range(args.num_classes):
        ...
        # 确保 idx_class 是一个 NumPy 数组
        # if isinstance(idx_class, bool):
        #     idx_class = np.array([idx_class])
        # elif not isinstance(idx_class, np.ndarray):
        #     idx_class = np.array(idx_class)
        #
        # # 确保 idx_class 的形状与 agreement_measure 一致
        # if idx_class.shape[0] != agreement_measure.shape[0]:
        #     idx_class = np.full(agreement_measure.shape, idx_class)
    ...
    for i in range(args.num_classes):
        ...
        agreement_measure = agreement_measure.to(device)
        idx_class = idx_class.to(device)
        top_clean_class_relative_idx = top_clean_class_relative_idx.to(device)
    ...
    return selected_examples.cuda(),final_selected_pairs.contiguous(), index_selected